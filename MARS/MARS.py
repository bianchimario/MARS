...
class MARS(BaseEstimator, TransformerMixin):
    def __init__(self, num_shapelets, max_len, min_len, async_limit=None, seed=None, indexes=False, shapelet_indexes = True):
        ...
        self.shapelets = None
        self.n_jobs = -1
        self.shapelets_indexes = shapelet_indexes
...
# ---------------------- Getting the shapelets ---------------------- 
    ...
    def get_random_shapelets(self, time_series_dataset):
        ...
        shapelets = []
        random_ts = random.sample(range(0, len(time_series_dataset)), self.num_shapelets) # indexes of the time series that generate a shapelet
